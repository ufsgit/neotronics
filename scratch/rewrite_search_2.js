...
const files = [
  'frontend/src/app/modules/admin/Price_Request/Price_Request.component.html',
  'frontend/src/app/modules/admin/Quotation_Confirmation/Quotation_Confirmation.component.html'
];
...
for (const file of files) {
  ...
  const isPriceRequest = file.includes('Price_Request');
  const isQuotation = file.includes('Quotation_Confirmation');
  
  const searchFunctionName = isPriceRequest ? 'Search_Price_Request' : 'Search_Quotation_Confirmation';
  const labelCustomer = isQuotation ? 'Customer' : 'Supplier';
  const labelDocNo = isPriceRequest ? 'Price_Request No' : 'Quotation No';
  ...
  let newBlock = `
            <div class="row mb-3 card-body" style="align-items: flex-end; border: 0; border-radius: 6px; margin-left: 2px; color: #333333; background: #fff; width: 100%; padding: 15px; background-color: #f1f1f1; box-shadow: 0 2px 2px 0 rgba(0, 0, 0, 0.14), 0 3px 1px -2px rgba(0, 0, 0, 0.2), 0 1px 5px 0 rgba(0, 0, 0, 0.12);">
                <div class="row w-100 mb-3" style="margin-left: 0;">
                    ...
                    <!-- Customer / Supplier -->
                    <div class="col-md-5">
                        <label class="mb-1" style="display: block; font-weight: 500; font-size: 13px;">${labelCustomer}</label>
                ...
                <div class="row w-100" style="margin-left: 0; align-items: flex-end;">
                    <!-- Document No -->
                    <div class="col-md-3">
                        <label class="mb-1" style="display: block; font-weight: 500; font-size: 13px;">${labelDocNo}</label>
                    ...
                    <div class="col-md-2" style="display:none;">
                        <label class="mb-1" style="display: block; font-weight: 500; font-size: 13px;">Part No</label>
                    ...
                    <!-- Buttons -->
                    <div class="col-md-4" style="display: flex; gap: 10px; justify-content: flex-end; align-items: flex-end; height: 60px;">
                        <button mat-raised-button type="button" (click)="${searchFunctionName}()" class="btn btn-danger m-0" style="height: 40px; border-radius: 8px; padding: 0 20px;">

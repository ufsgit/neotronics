...
var Stock_Take_Master=
{ 
    Save_Stock_Take_Master: async function (Stock_Take_Master_) {
        console.log('Stock_Take_Master: ',Stock_Take_Master_)
         return new Promise(async (rs,rej)=>{
           const pool = db.promise();
           let result1;
           var Stock_Take_Details_ = JSON.stringify(Stock_Take_Master_.Stock_Take_Details);
           var Stock_Take_Details_value_=0; 
           if(Stock_Take_Details_ !=undefined || Stock_Take_Details_ !=null || Stock_Take_Details_!="")  
            Stock_Take_Details_value_=1;

           console.log('Stock_Take_Master: ',Stock_Take_Details_)
           console.log('Stock_Take_Master: ',Stock_Take_Details_value_)

           var connection = await pool.getConnection();
           try {
            const result1 = await(new storedProcedure('Save_Stock_Take_Master',[Stock_Take_Master_.Stock_Take_Master_Id,
                Stock_Take_Master_.Stock_Take_Name_Id,Stock_Take_Master_.Stocktakename,Stock_Take_Master_.EntryDate,Stock_Take_Master_.User_Id,
                Stock_Take_Master_.User_Name,Stock_Take_Master_.Description,Stock_Take_Details_,Stock_Take_Details_value_], 
                connection)).result();
                console.log(result1) 
            ...
            rej(err);
            var result2=[{'Stock_Take_Master_Id_':0}]      
            rs(result2);
          }
          finally 
          {
...
Save_Stock_Add_Master1: async function (Stock_Add_Master_) 
    ...
    Search_Stock_Take_Master:function(Is_Date_Check_,From_date_,To_date_,Stock_Take_Name_Id_,Login_User_Id_,callback)
    {

        return db.query("CALL Search_Stock_Take_Master(@Is_Date_Check_ :=?,@From_date_ :=?,@To_date_  :=?,@Stock_Take_Name_Id_ :=?,@Login_User_Id_ :=?)",
         [Is_Date_Check_,From_date_,To_date_,Stock_Take_Name_Id_,Login_User_Id_],callback);
    },

    Get_Stock_Take_Details:function(Stock_Take_Master_Id_,callback)
        { 
        return db.query("CALL Get_Stock_Take_Details(@Stock_Take_Master_Id_ :=?)",[Stock_Take_Master_Id_],callback);
        } ,
...
module.exports=Stock_Take_Master;

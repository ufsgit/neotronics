...
const getRequirementTemplate = (data) => {
    ...
    let itemsHtml = details.map((item, index) => `
        <tr style="page-break-inside: avoid;">
            ...
            <td style="border: 1px solid #000;">
                <div style="font-weight: bold;">${item.ItemName || item.Item_Name || ''}</div>
                <div style="font-size: 10px; color: #555; margin-top: 2px;">${item.Description || item.Description1 || ''}</div>
            </td>
            <td style="text-align: center; border: 1px solid #000;">${item.Quantity || 0}</td>
            <td style="text-align: center; border: 1px solid #000;">${item.UnitName || item.Unit_Name || 'Nos'}</td>
            <td style="text-align: right; border: 1px solid #000;">${formatCurrency(item.UnitPrice || item.Rate)}</td>
            <td style="text-align: right; border: 1px solid #000; font-weight: bold;">${formatCurrency((Number(item.Quantity) || 0) * (Number(item.UnitPrice || item.Rate) || 0))}</td>
        </tr>
    `).join('');
...
<head>
    <meta charset="UTF-8">
    <title>Requirement - ${master.RequirementNo || ''}</title>
    <style>
        @page {
            size: A4;
            margin: 15mm;
...
<body>
    <div class="container">
        ...
        <div class="doc-title">Requirement</div>

        <!-- Info Section -->
        <div class="info-section">
            <div class="customer-box">
                <div class="section-label">Requirement From</div><br>
            ...
            <div class="details-box">
                <div class="section-label">Requirement Details</div><br>
                <strong>No: ${master.RequirementNo || ''}</strong><br>
...
module.exports = { getRequirementTemplate };

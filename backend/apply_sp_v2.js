...
const query1 = "DROP PROCEDURE IF EXISTS `Save_Sales_Order_Master`;";
const query2 = `
...
connection.connect((err) => {
    ...
    connection.query(query1, (err) => {
        if (err) { console.error('Error dropping SP:', err.message); connection.end(); return; }
        console.log('SP dropped successfully');
        connection.query(query2, (err) => {
            if (err) { console.error('Error creating SP:', err.message); }
            else { console.log('SP created successfully'); }
            connection.end();
        });
    });
});

...
export class Debit_NoteComponent implements OnInit, AfterViewInit  {
...
Save_Purchase(Printstatus:number)
{
  ...
  console.log("Before Debit Note Save API call");
  this.issLoading = true;

  this.Purchase_Master_Service_.Save_Purchase_Master(this.Purchase_Master_)
  .pipe(
    finalize(() => {
      console.log("Debit Note Save Finalize executed");
      this.issLoading = false;
  ...
  .subscribe({
    next: (Save_status) => {
      console.log("Debit Note Save API Response:", Save_status);
      ...
      const resultId = Number(Save_status[0][0].Purchase_Master_Id_);
      if (resultId > 0) {
        this.Purchase_Master_.Purchase_Master_Id = resultId;
        this.Purchase_Master_Id_Edit = resultId;
        this.Edit_Sales = 1;
        ...
        this.dialogBox.open(DialogBox_Component, {
          panelClass: 'Dialogbox-Class',
          data: { Message: 'Please choose another Invoice No. (Duplicate)', Type: "2" }
        });
      } else {
        this.dialogBox.open(DialogBox_Component, {
          panelClass: 'Dialogbox-Class',
    ...
    error: (error) => {
      console.error("Debit Note Save API ERROR:", error);
      this.dialogBox.open(DialogBox_Component, {
        panelClass: 'Dialogbox-Class',

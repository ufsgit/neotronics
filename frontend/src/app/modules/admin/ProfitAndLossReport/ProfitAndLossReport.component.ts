...
@Component({
        selector: 'app-ProfitAndLossReport',
        templateUrl: './ProfitAndLossReport.component.html',
        styleUrls: ['./ProfitAndLossReport.component.css'],
        providers: [
...
export class ProfitAndLossReportComponent implements OnInit {
        ...
        Company_Data:Company[];currentDate;FromDate;ToDate
...
Search_ProfitAndLoss()
        {
 

        this.issLoading=true;
         debugger
        this.Receipt_Voucher_Service_.Search_ProfitAndLoss(moment(this.Search_FromDate).format('YYYY-MM-DD'),
        moment(this.Search_ToDate).format('YYYY-MM-DD'),this.User_Type_Id, this.Login_User ).subscribe(Rows => {
                
    debugger
        this.ProfitLoss_Data=Rows[0];
        this.Total_Entries=this.ProfitLoss_Data.length;
        
        if (Rows != null) {
                this.Client_Accounts_Data_ = Rows[0];
                this.Print_Client1_ = this.Client_Accounts_Data_[0];           
            }
        if(this.ProfitLoss_Data.length==0)
        {

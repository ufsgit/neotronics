...
Sales_Master_Data1:[];UnitName="";Quantity=0;SaleRate=0;discount=0;unitDiscount=0;TotalAmount=0;availablity=0;Discount_Description=0;subTotal=0;POnumber;QuotNo;partNo;
Sales_Master_1:Sales_Master= new Sales_Master();
Address1 ='';Address2 ='';Address3 ='';Address4 ='';Attention;Employee;totalAmount=0;Total=0;totalDiscount=0;amount1=0;amount2=0;amount3=0;quotationId = 0;
...
ngOnInit() 
{
  ...
  this.Sales_Master_Id = Number(localStorage.getItem('Sales_Master_Id'))
  localStorage.removeItem('Sales_Master_Id');
...
if(this.quotationId <= 0 && this.PerformaInvoiceId <= 0 && this.DeliveryOrderMaster_Id <= 0 && this.Sales_Master_Id <= 0)
{
    this.Search_Sales(false);
}
}
Load_Company() 
  {   
    ...
    this.salesquotationmaster_Service_.Get_salesquotationmaster (this.quotationId).subscribe(result=>
       ...
       // this.Sales_Master_Data=result[0];
        ...
        // this.Sales_Master_.VAT_percentage = result[0][0].VAT_Percentage
         ...
         this.Sales_Master_.Account_Party_Id=result[0][0].Account_Party_Id;
        // this.Sales_Master_.SalesQuotationMaster_Id=result[0][0].SalesQuotationMaster_Id;
         this.Sales_Master_.CurrencyId=result[0][0].CurrencyId;
         this.Sales_Master_.Payment_Term_Description=result[0][0].Payment_Term_Description;
         this.Sales_Master_.TypeId=result[0][0].TypeId;

        this.Sales_Master_.LPONo=result[0][0].POnumber;   
        this.Sales_Master_.PaymentTermValue=result[0][0].PaymentTermValue;
        this.Sales_Master_.SalesQuotationMaster_Id=this.quotationId;
         this.Sales_Master_.Payment_Term_Description=result[0][0].Payment_Term_Description;
         this.Sales_Master_.CurrencyId=result[0][0].CurrencyId;
         this.Sales_Master_.TypeId=result[0][0].TypeId;
         this.Sales_Master_.Account_Party_Id=result[0][0].Account_Party_Id;
         this.Sales_Master_.Customer_Name=result[0][0].Customer_Name;
        // this.Sales_Master_.Discount_Description=result[0][0].Discount_Description;
        // this.Sales_Master_.Charge1=result[0][0].Charge1;
        // this.Sales_Master_.Charge2=result[0][0].Charge2;        
        // this.Sales_Master_.Employee=result[0][0].AttendEmployee;
        // this.Sales_Master_.Charge1per=result[0][0].Charge1per;
        this.Sales_Master_.VAT_percentage = result[0][0].VAT_Percentage;
        debugger;
            // if(this.Sales_Master_.VAT_percentage == undefined || this.Sales_Master_.VAT_percentage == null)
            // this.Sales_Master_.VAT_percentage = this.Default_Vat_Percentage;

        // this.Sales_Master_.PriceBasis=result[0][0].PriceBasis;
        // this.Sales_Master_.Delivery=result[0][0].Delivery;
        // this.Sales_Master_.Validity=result[0][0].Validity;
       // this.Sales_Master_.Description2=result[0][0].Description1;  
        ...
        this.Sales_Master_.SupplyDate=new Date().toString();
        this.Sales_Master_.SupplyPrintDate = this.Sales_Master_.SupplyDate;
        this.Sales_Master_.SupplyPrintDate = this.formatPrintDate(this.Sales_Master_.SupplyPrintDate);
        this.Sales_Master_.SupplyDate=this.formatDate(this.Sales_Master_.SupplyDate);
        ...
        {
            this.Sales_Master_.DueDate = new Date(new Date().setDate(new Date().getDate() + result[0][0].PaymentTermValue)).toString();
            this.Sales_Master_.DuePrintDate = this.Sales_Master_.DueDate;
            this.Sales_Master_.DuePrintDate = this.formatPrintDate(this.Sales_Master_.DuePrintDate);
            this.Sales_Master_.DueDate=this.formatDate(this.Sales_Master_.DueDate);
        }
        else
        {
            this.Sales_Master_.DueDate=new Date().toString();
            this.Sales_Master_.DuePrintDate = this.Sales_Master_.DueDate;
            this.Sales_Master_.DuePrintDate = this.formatPrintDate(this.Sales_Master_.DuePrintDate);
            this.Sales_Master_.DueDate=this.formatDate(this.Sales_Master_.DueDate);
        }   
...
loadPerformaInvoice(){
    ...
    this.salesquotationmaster_Service_.Get_salesPerformaInvoicemaster(this.PerformaInvoiceId).subscribe(result=>{      
        ...
        this.Sales_Master_.PaymentTermValue=result[0][0].PaymentTermValue;
        this.Sales_Master_.SalesQuotationMaster_Id=result[0][0].SalesQuotationMaster_Id;
        this.Sales_Master_.PerformaInvoiceMaster_Id=this.PerformaInvoiceId;
         this.Sales_Master_.Payment_Term_Description=result[0][0].Payment_Term_Description;
         this.Sales_Master_.CurrencyId=result[0][0].CurrencyId;
         this.Sales_Master_.TypeId=result[0][0].TypeId;
        ...
        this.Customer_Temp.Client_Accounts_Id=this.Sales_Master_.Account_Party_Id;
        this.Customer_Temp.Client_Accounts_Name=this.Sales_Master_.Customer;
        this.Customer_=this.Customer_Temp;

        this.Customer_.Client_Accounts_Id=this.Sales_Master_.Account_Party_Id;
        this.Customer_.Client_Accounts_Name=this.Sales_Master_.Customer;
        ...
        this.Sales_Master_.Employee = result[0][0].PreparedBy;
...
{   
    this.Sales_Master_.EntryDate=this.formatDate(this.Sales_Master_.EntryDate);
    this.Sales_Master_.PrintDate = this.formatPrintDate(this.Sales_Master_.EntryDate);
...
debugger;
  if (this.Sales_Master_.Charge1per != '')
    if (this.Sales_Master_.Charge1per != null)
        if (this.Sales_Master_.Charge1per != undefined)
            if (this.Sales_Master_.Charge1per.toString() != '0')
                if (this.Sales_Master_.Charge1per.toString() != 'null')
                    if (this.Sales_Master_.Charge1per.toString() != 'undefined') 
                        if (this.Sales_Master_.Charge1per.toString() != '0.000')
                            if (this.Sales_Master_.Charge1per.toString()!='0.00')
                        {
...
if (this.Sales_Master_.charge1_Amount != 0)
    if (this.Sales_Master_.charge1_Amount != null)
        if (this.Sales_Master_.charge1_Amount != undefined)
            if (this.Sales_Master_.charge1_Amount.toString() != '0')
                if (this.Sales_Master_.charge1_Amount.toString() != 'null')
                    if (this.Sales_Master_.charge1_Amount.toString() != 'undefined')
                        if (this.Sales_Master_.charge1_Amount.toString() != '0.000') 
                             if (this.Sales_Master_.charge1_Amount.toString() != '0.00') 
                            {
...
debugger;
if (this.Sales_Master_.charge2_Amount != 0)
    if (this.Sales_Master_.charge2_Amount != null)
        if (this.Sales_Master_.charge2_Amount != undefined)
            if (this.Sales_Master_.charge2_Amount.toString() != '0')
                if (this.Sales_Master_.charge2_Amount.toString() != 'null')
                    if (this.Sales_Master_.charge2_Amount.toString() != 'undefined')
                        if (this.Sales_Master_.charge2_Amount.toString() != '0.000') 
                             if (this.Sales_Master_.charge2_Amount.toString() != '0.00') 
                            {
...
debugger;
if (this.Sales_Master_.VAT_Amount != 0)
    if (this.Sales_Master_.VAT_Amount != null)
        if (this.Sales_Master_.VAT_Amount != undefined)
            if (this.Sales_Master_.VAT_Amount.toString() != '0')
                if (this.Sales_Master_.VAT_Amount.toString() != 'null')
                    if (this.Sales_Master_.VAT_Amount.toString() != 'undefined')
                            if (this.Sales_Master_.VAT_Amount.toString() != '0.000') 
                             if (this.Sales_Master_.VAT_Amount.toString() != '0.00') 
                         {
                        this.printVAT_Amount = true;
                    }

if (this.Sales_Master_.Discount_Description != '')
    if (this.Sales_Master_.Discount_Description != null)
        if (this.Sales_Master_.Discount_Description != undefined)
            if (this.Sales_Master_.Discount_Description.toString() != '0')
                if (this.Sales_Master_.Discount_Description.toString() != 'null')
                    if (this.Sales_Master_.Discount_Description.toString() != 'undefined') 
                          if (this.Sales_Master_.Discount_Description.toString() != '0.000') 
                             if (this.Sales_Master_.Discount_Description.toString() != '0.00') 
                        {
                        this.printDiscount_Description = true;
                    }


if (this.Sales_Master_.Additional_Discount != 0)
    if (this.Sales_Master_.Additional_Discount != null)
        if (this.Sales_Master_.Additional_Discount != undefined)
            if (this.Sales_Master_.Additional_Discount.toString() != '0')
                if (this.Sales_Master_.Additional_Discount.toString() != 'null')
                    if (this.Sales_Master_.Additional_Discount.toString() != 'undefined') 
                          if (this.Sales_Master_.Additional_Discount.toString() != '0.000') 
                             if (this.Sales_Master_.Additional_Discount.toString() != '0.00') 
                        {
                        this.printAdditional_Discount = true;
                    }

if (this.Sales_Master_.Roundoff_Amt != 0)
    if (this.Sales_Master_.Roundoff_Amt != null)
        if (this.Sales_Master_.Roundoff_Amt != undefined)
            if (this.Sales_Master_.Roundoff_Amt.toString() != '0')
                if (this.Sales_Master_.Roundoff_Amt.toString() != 'null')
                    if (this.Sales_Master_.Roundoff_Amt.toString() != 'undefined') 
                         if (this.Sales_Master_.Roundoff_Amt.toString() != '0.000') 
                             if (this.Sales_Master_.Roundoff_Amt.toString() != '0.00') 
                        {
...
Clr_Sales_Master()
{
  ...
  this.Sales_Master_.DueDate=new Date().toString();
  this.Sales_Master_.DueDate=this.formatDate(this.Sales_Master_.DueDate);
  this.Sales_Master_.Invoice_No="";
  this.Sales_Master_.CurrencyId=0;
  this.Sales_Master_.LPONo="";
  this.Sales_Master_.DONo="";
  this.Sales_Master_.Charge1="";
  this.Sales_Master_.PaymentTerms = null;
  this.Sales_Master_.Payment_Term_Description = null;
  this.Sales_Master_.PackingListNumber = null;
  this.Sales_Master_.Description1 = "";
  this.Sales_Master_.Discount_Description = null;
  this.Sales_Master_.Charge1per = null;
  this.Sales_Master_.Charge2 = "";
  this.Sales_Master_.VAT_percentage = null;

  this.SalesQuotationMaster_Id = 0;
  this.Sales_Master_.SalesQuotationMaster_Id = 0;


  this.Sales_Master_.Additional_Discount = null;
  this.Sales_Master_.charge1_Amount = null;
  this.Sales_Master_.charge2_Amount = null;
  this.Sales_Master_.TotalDiscount = null
  this.Sales_Master_.VAT_Amount = null;
  this.Sales_Master_.Total_Amount = null;
  this.Sales_Master_.Roundoff_Amt = null;
  this.Sales_Master_.Amount_In_Words = "";
  this.Sales_Master_.NetTotal=null;
  this.Sales_Master_.Cess=0;
  this.Sales_Master_.RoundOff=null;
  this.Sales_Master_.TotalAmount=0;
  this.Sales_Master_.Description2="";
  this.Sales_Master_.Address1="";
  this.Sales_Master_.Address2="";
  this.Sales_Master_.Address3="";
  this.Sales_Master_.Address4="";
  this.Sales_Master_.Mobile="";
  this.Sales_Master_.Customer_Name="";
  this.Sales_Master_.Employee="";
  this.Sales_Master_.KindAttend="";    
  this.Sales_Master_.GrandTotal=null;
  this.Sales_Master_.Transportation_Gst=0;
  this.Sales_Master_.Handling_Gst=0;
  this.Sales_Master_.Transportation_Total=0;
  this.Sales_Master_.Handling_Total=0;
  this.Sales_Master_.Vehicle_No="";
  this.Sales_Master_.Driver_Name="";
  this.Sales_Master_.Mobile_No = "";
  this.Sales_Master_.Delivery_Address1 = "";
  this.Sales_Master_.Delivery_Address2 = "";
  this.Sales_Master_.Delivery_Address3 = "";
  this.Sales_Master_.Delivery_Address4 = "";
  this.Sales_Master_.GSTNo = "";
...
Clr_Sales_Details()
{
  ...
  this.Sales_Details_.GroupId=0;
  this.Sales_Details_.GroupName="";
  this.Sales_Details_.UnitId=0;
  this.Sales_Details_.UnitName="";
  this.Sales_Details_.PurchaseRate=null;
  this.Sales_Details_.SaleRate=null;
  this.Sales_Details_.MRP=0;
  this.Sales_Details_.TaxAmount=0;
  this.Sales_Details_.Stock=0;
  this.Sales_Details_.HSNCODE="";
  this.Sales_Details_.SaleTax=0;
  this.Sales_Details_.Quantity=null;
  this.Sales_Details_.Discount=null;
  this.Sales_Details_.NetValue=null;
  this.Sales_Details_.Description="";
  this.Barcode_=null;
  this.ItemCodeData =null;
  this.Stock_=null;
  this.UnitName = null;
  this.Quantity = null;
  this.Sales_Details_.UnitPrice = null;
  this.discount = null;
...
Search_Item_Typeahead(event: any)
{
  ...
  this.Sales_Details_.ItemName=Value;
...
Customer_Change( Customer_T_)
{
    ...
    this.Sales_Master_.Account_Party_Id=this.Customer_.Client_Accounts_Id;
    ...
    this.Address2 = this.Sales_Master_.Address2;
    this.Address3 = this.Sales_Master_.Address3;
    this.Address4 = this.Sales_Master_.Address4;
    this.Vatin = this.Sales_Master_.GSTNo;
}
...
Item_Name_Change(Item_sl:Stock){
  ...
  this.Sales_Details_.ItemId=Item_sl.ItemId;
  this.Sales_Details_.ItemCode=Item_sl.Item_Code;

  this.Sales_Details_.ItemName=Item_sl.ItemName;
  this.Sales_Details_.UnitId=Item_sl.UnitId;
  this.Sales_Details_.UnitName=Item_sl.UnitName;
  this.Sales_Details_.MRP=Item_sl.MRP;
  this.Sales_Details_.PurchaseRate=Item_sl.PurchaseRate;

  this.Sales_Details_.GroupId=Item_sl.GroupId;
  this.Sales_Details_.GroupName=Item_sl.GroupName;
  this.Sales_Details_.HSNMasterId=Item_sl.HSNMasterId;
  this.Sales_Details_.HSNCODE=Item_sl.HSNCODE;
  this.Sales_Details_.Stock_Id=Item_sl.Stock_Id;
  this.Sales_Details_.SaleTax=Item_sl.SaleTax; 
...
if(this.Sales_Details_.Discount == undefined || this.Sales_Details_.Discount == null)
    this.Sales_Details_.Discount = 0;
if(this.Sales_Details_.Item_Discount_Amount == undefined || this.Sales_Details_.Item_Discount_Amount == null)
    this.Sales_Details_.Item_Discount_Amount = 0;
if(this.Sales_Details_.Discount==undefined)
    this.Sales_Details_.Discount=0;
this.Sales_Details_.Unit_Discount = (Number(this.Sales_Details_.UnitPrice) * Number(this.Sales_Details_.Discount))/ 100;
this.Sales_Details_.Unit_Discount = Number(this.Sales_Details_.Unit_Discount.toFixed(3));

this.Sales_Details_.Item_Discount_Amount =Number(this.Sales_Details_.Unit_Discount) * Number(this.Sales_Details_.Quantity);
this.Sales_Details_.Item_Discount_Amount = Number(this.Sales_Details_.Item_Discount_Amount.toFixed(3));

this.Sales_Details_.Amount = Number(this.Sales_Details_.Quantity) * Number(this.Sales_Details_.UnitPrice);
this.Sales_Details_.Amount =Number(this.Sales_Details_.Amount.toFixed(3));

 this.Sales_Details_.TaxableAmount = Number(this.Sales_Details_.Amount) - Number(this.Sales_Details_.Item_Discount_Amount);
 this.Sales_Details_.TaxableAmount =Number(this.Sales_Details_.TaxableAmount.toFixed(3));
if(this.Sales_Details_.SaleTax==undefined || this.Stock_.SaleTax==null)
    this.Sales_Details_.SaleTax=0;
this.Sales_Details_.TaxAmount = Number(this.Sales_Details_.TaxableAmount) * Number(this.Sales_Details_.SaleTax) /100;
this.Sales_Details_.NetValue= Number(this.Sales_Details_.TaxableAmount) + Number(this.Sales_Details_.TaxAmount);

this.Sales_Details_.Item_Discount_Amount=Number(this.Sales_Details_.Item_Discount_Amount.toFixed(3));    
this.Sales_Details_.Amount=Number(this.Sales_Details_.Amount.toFixed(3));    
this.Sales_Details_.TaxableAmount=Number(this.Sales_Details_.TaxableAmount.toFixed(3));
this.Sales_Details_.TaxAmount=Number(this.Sales_Details_.TaxAmount.toFixed(3));  
this.Sales_Details_.NetValue= Number(this.Sales_Details_.NetValue.toFixed(3));
...
Search_Sales(showNoDetails: boolean = true)
{
...
this.Login_User_Id).subscribe(Rows => {
  ...
  if(this.Sales_Master_Data.length>0)
  {
      ...
      {
          this.Sales_Master_Total_Amount=Number(this.Sales_Master_Total_Amount)+Number(this.Sales_Master_Data[i].NetTotal);
          this.Sales_Master_Total_Amount= Number(this.Sales_Master_Total_Amount.toFixed(3));
      }
  }
  this.Total_Entries=this.Sales_Master_Data.length;
  if(showNoDetails && this.Sales_Master_Data.length==0)
  {
...
Next_Invoice_Page() {
  if (this.Page_Index < this.Invoice_Total_Pages - 1) {
      this.Page_Index++;
  }
}
Add_Sales_Details()
...
{   
  ...
  else if(this.Sales_Details_.Quantity==undefined || this.Sales_Details_.Quantity==null || this.Sales_Details_.Quantity==0 )
  {
  ...
  else if(this.Sales_Details_.UnitPrice==undefined || this.Sales_Details_.UnitPrice==null || this.Sales_Details_.UnitPrice==0 )
  {
...
//   this.Sales_Details_.ItemCode = this.Barcode_.Item_Code;
...
//   this.Sales_Details_.Stock_Id = this.Stock_.Stock_Id;
//   this.Sales_Details_.HSNMasterId = this.Stock_.HSNMasterId;
//   this.Sales_Details_.HSNCODE = this.Stock_.HSNCODE;
//   this.Sales_Details_.UnitPrice = this.SaleRate;
//   this.Sales_Details_.Quantity = this.Quantity;
//   this.Sales_Details_.Amount = this.TotalAmount;
//   this.Sales_Details_.Discount = this.discount;
//   this.Sales_Details_.Unit_Discount = this.unitDiscount;
//   this.Sales_Details_.SaleTax = this.Stock_.SaleTax;
//   this.Sales_Details_.TaxableAmount = this.TotalAmount-this.discount;
//   this.Sales_Details_.TaxAmount = this.Sales_Details_.TaxableAmount * (this.safeNumber(this.Stock_.SaleTax) / 100)
//   this.Sales_Details_Data.push(Object.assign({}, this.Sales_Details_));
if(this.Sales_Details_.Stock_Id>0)
    this.Sales_Details_.Stock_Id=this.Sales_Details_.Stock_Id;
else
    this.Sales_Details_.Stock_Id=0;
    if(this.Barcode_.Item_Code!=undefined && this.Barcode_.Item_Code!=null)
        ...
        else if(this.Barcode_!=undefined && this.Barcode_!=null)
        {
            ...
            this.Sales_Details_.ItemCode=Barcode_string;
        }


if (this.Sales_Details_Data == undefined)
    this.Sales_Details_Data = [];
var inserted = 0;
this.Sales_Details_.UnitPrice = parseFloat(Number(this.Sales_Details_.UnitPrice).toFixed(3));
this.Sales_Details_.Quantity = parseFloat(Number(this.Sales_Details_.Quantity).toFixed(3));
if (this.Sales_Details_Index >= 0) 
    {
        this.Sales_Details_Data[this.Sales_Details_Index] = Object.assign({}, this.Sales_Details_);
    }
...
debugger;
// for (var i = 0; i < this.Sales_Details_Data.length; i++) {
//     if (this.Sales_Details_Data[i].Stock_Id == this.Sales_Details_.Stock_Id) {
//         if (this.Sales_Details_Index == i) {
//             this.Sales_Details_Data[i] = Object.assign({}, this.Sales_Details_)// this.Sales_Details_;
//         }
//         else {
//             this.Sales_Details_.TaxableAmount = this.TotalAmount-this.discount;
//             this.Sales_Details_.TaxAmount = this.Sales_Details_.TaxableAmount * (this.safeNumber(this.Stock_.SaleTax) / 100);
//             this.Sales_Details_.Item_Discount_Amount = Number(this.Sales_Details_.UnitPrice) * this.safeNumber(this.Sales_Details_.Discount)/100;
//             //this.Sales_Details_Data.push(Object.assign({}, this.Sales_Details_));
//             this.Sales_Details_Data[i] = Object.assign({}, this.Sales_Details_)
//         }
//         if (this.Sales_Details_Index != i && this.Sales_Details_Index >= 0) {
//             this.Sales_Details_Data.splice(this.Sales_Details_Index, 1)
//         }
//         inserted = 1;
//         i = this.Sales_Details_Data.length;
//         break;
//     }
//     }
    ...
    // if (inserted == 0) {
    //     if (this.Sales_Details_Index >= 0) {
    //         this.Sales_Details_Data[this.Sales_Details_Index] = Object.assign({}, this.Sales_Details_)// this.Sales_Details_;
    //     }
    //     else {
    //         this.Sales_Details_Data.push(Object.assign({}, this.Sales_Details_));
    //     }
...
Save_Sales(Printstatus:number)
{
  ...
  if(this.Sales_Details_Data == undefined || this.Sales_Details_Data == null || this.Sales_Details_Data.length == 0 || this.Sales_Details_Data.length == undefined )
  {
  ...
  {
    if(this.Sales_Details_Data[i].UnitPrice == 0)
    {
        unitPriceCheck = 1;
    }
  }
  ...
  if(this.Sales_Master_.DeliveryOrderMaster_Id == null || this.Sales_Master_.DeliveryOrderMaster_Id == undefined)
  {
    this.Sales_Master_.DeliveryOrderMaster_Id = 0;
  }
//   if(this.Attention)
//   this.Sales_Master_.KindAttend=this.Attention.User_Details_Id;
//   if(this.Employee)
//   this.Sales_Master_.Employee = this.Employee.User_Details_Id
//   this.Sales_Master_.Payment_Term_Description = this.PaymentTerm.Payment_Term_Description;
//   this.Sales_Master_.PaymentTerms = this.PaymentTerm.payment_Term_ID;

debugger;
if(this.PaymentTerm == undefined || this.PaymentTerm == null){
    this.Sales_Master_.Payment_Term_Description = 0;
    this.Sales_Master_.PaymentTerms = "";
}else{
    this.Sales_Master_.Payment_Term_Description = this.PaymentTerm.payment_Term_ID;
    this.Sales_Master_.PaymentTerms = this.PaymentTerm.Payment_Term_Description;
}

  this.Sales_Master_.EntryDate = this.New_Date(new Date(moment(this.Sales_Master_.EntryDate).format('YYYY-MM-DD')));
  this.Sales_Master_.SupplyDate = this.New_Date(new Date(moment(this.Sales_Master_.SupplyDate).format('YYYY-MM-DD')));
...
Edit_Sales_Details(Sales_Details_ed:Sales_Details,index)
{   
  this.Sales_Details_Index=index;
  this.Sales_Details_ = {...Sales_Details_ed}
  ...
  this.Sales_Details_=Object.assign({},Sales_Details_ed);
...
//this.Barcode_.ItemName=this.Stock_.ItemName;
    ...
    this.Sales_Details_.PurchaseRate=this.Barcode_.PurchaseRate;
    this.Sales_Details_.Stock=this.Barcode_.Quantity;
    this.Sales_Details_.UnitPrice=this.Barcode_.SaleRate;
    this.Sales_Details_.ItemName=this.Barcode_.ItemName;
    this.Sales_Details_.GroupId=this.Barcode_.GroupId;
    this.Sales_Details_.GroupName=this.Barcode_.GroupName;
    this.Sales_Details_.HSNMasterId=this.Barcode_.HSNMasterId;
    ...
    this.Sales_Details_.SaleTax=this.Barcode_.SaleTax;
    this.Sales_Details_.ItemCode=this.Barcode_.Item_Code;
    this.Sales_Details_.Country_Id=this.Barcode_.Country_Id;
    this.Sales_Details_.Country_Name=this.Barcode_.Country_Name;
...
Delete_Sales_Detail(itemIndex){
  this.Sales_Details_Data.splice(itemIndex, 1);
// if( this.Sales_Details_Data.length>21){
  this.addBlankRows();//}
  this.Final_Amounts();
}

Edit_Sales_Detail(Sales_Details_ed:Sales_Details,index)
{
...
//   this.Sales_Details_Data.splice(index, 1);
    ...
    // this.Stock_.ItemId = Sales_Details_ed.ItemId;
    // this.Stock_.ItemName= Sales_Details_ed.ItemName ;
    // this.Stock_.UnitName =  Sales_Details_ed.UnitName 
    // this.Stock_.UnitId = Sales_Details_ed.UnitId;
    // this.Stock_.GroupId = Sales_Details_ed.GroupId;
    // this.Stock_.GroupName = Sales_Details_ed.GroupName;  
    // this.Stock_.HSNMasterId = Sales_Details_ed.HSNMasterId;
    // this.Stock_.HSNCODE = Sales_Details_ed.HSNCODE;
    // this.Stock_.SaleTax = Sales_Details_ed.SaleTax;
...
newQuotation(){
  ...
  duplicate(){
    ...
    // this.Sales_Master_.Delivery_Address3 = '';
    // this.Sales_Master_.Delivery_Address4 = ''; 
    this.Sales_Master_.Invoice_No = '0'
...
Load_Delivery_Order_Master(){
    ...
    this.Sales_Master_Service_.Load_Delivery_Order_Master(this.DeliveryOrderMaster_Id).subscribe(result=>{
       ...
       // this.Sales_Master_Data=result[0];
        ...
        this.Sales_Master_.PaymentTermValue=result[0][0].PaymentTermValue;
        this.Sales_Master_.SalesQuotationMaster_Id=0;
        this.Sales_Master_.DeliveryOrderMaster_Id=this.DeliveryOrderMaster_Id;
         this.Sales_Master_.payment_Term_ID=result[0][0].Payment_Term_Description;
         this.Sales_Master_.Payment_Term_Description=result[0][0].Payment_Term_Description;
         this.Sales_Master_.CurrencyId=result[0][0].CurrencyId;
         this.Sales_Master_.TypeId=result[0][0].TypeId;
        // this.Sales_Master_.Discount_Description=result[0][0].Discount_Description;
        ...
        // this.Sales_Master_.Charge2=result[0][0].Charge2;
        // this.Sales_Master_.Charge1per=result[0][0].Charge1per;
        // this.Sales_Master_.DONo=result[0][0].DONo;
        // this.Sales_Master_.Employee=result[0][0].AttendEmployee;
        ...
        this.Sales_Master_Service_.Get_Delivery_Order_Details(this.Sales_Master_.DeliveryOrderMaster_Id).subscribe(Result => {           
            debugger;    
            if (Result != null) {
                this.Sales_Details_Data = Result[0];
                this.Final_Amounts();
                // if( this.Sales_Details_Data.length>21){
                this.addBlankRows();   //}             
                }                   
...
Load_SalesMaster()
  {
    ...
    this.issLoading = true;
      ...
      this.Sales_Master_Service_.Load_SalesMaster(this.Sales_Master_Id).subscribe(rows=>{
        ...
        debugger;
  this.Sales_Master_Id_Edit = this.Sales_Master_Id
  this.Sales_Master_Id = 0;
  this.Edit_Sales=1;
  this.Sales_Print = false;
  //this.Sales_Master_Data=rows[0]
  this.Sales_Master_=Object.assign({},rows[0][0]); 
  ...
  this.SalesQuotationMaster_Id = rows[0][0].SalesQuotationMaster_Id;
    ...
    this.Sales_Master_.Account_Party_Id=rows[0][0].Account_Party_Id;
    this.Sales_Master_.Customer_Name=this.Customer_.Client_Accounts_Name;

    this.Sales_Master_.Customer=this.Sales_Master_.Customer_Name; 
    this.Sales_Master_.LPONo=rows[0][0].LPONo;
...
DueDateChange()
{
    ...
    if(this.Sales_Master_.EntryDate)
        {
            if (!this.Sales_Master_.PaymentTermValue) {
                this.Sales_Master_.DueDate = this.Sales_Master_.EntryDate;
            } else {
                const entryDate = new Date(this.Sales_Master_.EntryDate); // Convert EntryDate to a Date object
                this.Sales_Master_.DueDate = new Date(entryDate.setDate(entryDate.getDate() + this.Sales_Master_.PaymentTermValue)).toString();
                this.Sales_Master_.DueDate = this.formatDate(this.Sales_Master_.DueDate);
            }
        }
    ...
    if(!this.Sales_Master_.EntryDate)
    {
        if(!this.Sales_Master_.PaymentTermValue)
            ...
            {
                this.Sales_Master_.DueDate = new Date(new Date().setDate(new Date().getDate() + this.Sales_Master_.PaymentTermValue)).toString();
                this.Sales_Master_.DueDate=this.formatDate(this.Sales_Master_.DueDate);
            } 
    }
...
{
    if(this.Sales_Master_.EntryDate)
    this.Sales_Master_.SupplyDate = this.Sales_Master_.EntryDate;
}
...
Calculate_Discount_Percent()
{
  ...
  debugger;
  if(Number(this.Sales_Master_.Additional_Discount)>0){
      this.Sales_Master_.Discount_Description = (this.Sales_Master_.Additional_Discount * 100/this.Sales_Master_.TotalAmount).toString();
      discountper = Number(this.Sales_Master_.Discount_Description).toFixed(3);
      this.Sales_Master_.Discount_Description = discountper; 
  }else{
      this.Sales_Master_.Discount_Description = '0.000'
  }
...
{      
    debugger
  this.Tot_Gross=0,this.Tot_discount=0,this.Tot_Net=0,this.Sales_Master_.TotalAmount=0;this.Sales_Master_.Basic_Discount=0
  for(var i = 0; i< this.Sales_Details_Data.length ; i++)
  {
    // this.Sales_Master_.TotalAmount = Number(this.Sales_Master_.TotalAmount) + Number(this.Sales_Details_Data[i].UnitPrice) * Number(this.Sales_Details_Data[i].Quantity);
    this.Sales_Master_.TotalAmount = Number(this.Sales_Master_.TotalAmount) + Number(this.Sales_Details_Data[i].Amount);

      this.Tot_discount = Number(this.Tot_discount)+  (Number(this.Sales_Details_Data[i].Unit_Discount) * Number(this.Sales_Details_Data[i].Quantity) );
      this.Tot_discount = Number(this.Tot_discount.toFixed(3));

      this.Sales_Master_.Basic_Discount = this.Sales_Master_.Basic_Discount + Number(this.Sales_Details_Data[i].Item_Discount_Amount)
      this.Sales_Master_.Basic_Discount = Number(this.Sales_Master_.Basic_Discount.toFixed(3));        
  ...
  if(this.addDiscCheck == 0)
    {
  if(Number(this.Sales_Master_.Discount_Description)>0){
      this.Sales_Master_.Additional_Discount = Number(this.Sales_Master_.TotalAmount) * (Number(this.Sales_Master_.Discount_Description)/ 100);
      this.Sales_Master_.Additional_Discount = Number(this.Sales_Master_.Additional_Discount.toFixed(3))
      this.addDiscCheck = 0;

  }else{
      this.Sales_Master_.Additional_Discount = 0
      this.addDiscCheck = 0;
...
if (this.Sales_Master_.VAT_percentage==undefined || this.Sales_Master_.VAT_percentage==null)
    this.Sales_Master_.VAT_percentage=0;
  //this.Sales_Master_.Discount_Description = (this.safeNumber(this.Sales_Master_.Additional_Discount) * 100)/this.Tot_Amount
  this.Sales_Master_.TotalDiscount = Number(this.Tot_discount)+ Number(this.Sales_Master_.Additional_Discount);
  this.Sales_Master_.TotalDiscount = Number(this.Sales_Master_.TotalDiscount.toFixed(3))
  if((this.Sales_Master_.Charge1per)>'0'){
      this.Sales_Master_.charge1_Amount = (Number(this.Sales_Master_.TotalAmount) - Number(this.Sales_Master_.Additional_Discount))* (Number(this.Sales_Master_.Charge1per)/100)
      this.Sales_Master_.charge1_Amount = Number(this.Sales_Master_.charge1_Amount.toFixed(3))
  }else{
      this.Sales_Master_.charge1_Amount =  0.000;
  }
  this.Total = Number(this.Sales_Master_.TotalAmount.toFixed(3))-Number(this.Sales_Master_.TotalDiscount.toFixed(3)) + 
  this.safeNumber(Number(this.Sales_Master_.charge2_Amount)) + Number(this.Sales_Master_.charge1_Amount.toFixed(3))
  this.Total = Number(this.Total.toFixed(3))
  this.Sales_Master_.VAT_Amount = 0.000;
  if(this.Sales_Master_.VAT_percentage>0){
      this.Sales_Master_.VAT_Amount = Number(this.Total) * Number(this.Sales_Master_.VAT_percentage/100)
  }
  this.Sales_Master_.VAT_Amount = Number(this.Sales_Master_.VAT_Amount.toFixed(3))    
  this.Sales_Master_.Total_Amount = this.Total + this.Sales_Master_.VAT_Amount
  this.Sales_Master_.Total_Amount = Number(this.Sales_Master_.Total_Amount.toFixed(3))
  this.Sales_Master_.NetTotal = Number((this.Sales_Master_.Total_Amount - this.safeNumber(this.Sales_Master_.Roundoff_Amt)).toFixed(3))
  this.Sales_Master_.NetTotal = Number(this.Sales_Master_.NetTotal.toFixed(3))
  this.Sales_Master_.TotalAmount = parseFloat(this.Sales_Master_.TotalAmount.toFixed(3));
  this.Sales_Master_.Amount_In_Words = this.numberToWordsIndianCurrency(this.Sales_Master_.NetTotal)
   debugger;
...
// Export() {
//     this.Sales_Master_Service_.exportExcel(this.Sales_Master_Data,"Invoice" );
//   }

  Export() {
    const filteredData = this.Sales_Master_Data.map((receipt: any, index: number) => {
        return {
            No: index + 1,                  
...
//     this.Sales_Details_Data.forEach((_, index) => {
...
//                     if(this. Sales_Details_Data[index].ItemName.length )
//           var alenght=  this. Sales_Details_Data[index].ItemName.length % 39;

//         console.log('this. Sales_Details_Data[index].ItemName.length: ', this. Sales_Details_Data[index].ItemName.length);
  ...
  addBlankRows(): void {
    ...
    this.Sales_Details_Data.forEach((_, index) => {
      ...
      if (cell) {
        ...
        if (cellThr1 < 375 && this.Sales_Details_Data.length < 14 ) {   

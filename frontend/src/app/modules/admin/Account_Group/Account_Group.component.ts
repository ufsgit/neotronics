...
export class Account_GroupComponent implements OnInit {
    ...
    Permissions: any;
    Account_Group_Edit: boolean = false;
    Account_Group_Save: boolean = false;
    Account_Group_Delete: boolean = false;
    ...
    Clr_Account_Group() {
        this.Account_Group_ = new Account_Group();
    ...
    Load_Account_Group(event: any) {
        ...
        this.Account_Group_Service_.Load_Account_Group(Value).subscribe(
            Rows => {
                if (Rows != null) {
    ...
    Search_Account_Group() {
        ...
        this.Account_Group_Service_.Search_Account_Group(this.Search_Account_Group_).subscribe(
            Rows => {
    ...
    Delete_Account_Group(Account_Group_Id, index) {
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result == 'Yes') {
                this.issLoading = true;
                this.Account_Group_Service_.Delete_Account_Group(Account_Group_Id).subscribe(
                    (Delete_status: any) => {
                        this.issLoading = false;
                        let status: any = Delete_status;
                        if (Array.isArray(status)) status = status[0];
                        if (Array.isArray(status)) status = status[0];
                        
                        // Handle the nested structure seen in the original code
                        let delStatus: any = status;
                        if (status && status.DeleteStatus_ && status.DeleteStatus_.data) {
                            delStatus = status.DeleteStatus_.data[0];
                        }

                        if (delStatus == 1 || (status && status.DeleteStatus == true)) {
                            this.Account_Group_Data.splice(index, 1);
                            const dialogRef = this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Deleted', Type: "false" } });
                            this.Search_Account_Group();
                        } else {
                            const msg = status.DeleteMessage || 'Cannot be Deleted, its already used';
                            const dialogRef = this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: msg, Type: "2" } });
                        }
                    },
                    Rows => {
                        this.issLoading = false;
    ...
    Save_Account_Group() {
        if (this.Account_Group_.Group_Name == null || this.Account_Group_.Group_Name == "" || this.Account_Group_.Group_Name == undefined) {
            const dialogRef = this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Enter the Group Name', Type: "3" } });
            ...
            this.issLoading = true;
            this.Account_Group_Service_.Save_Account_Group(this.Account_Group_).subscribe(
                (Save_status: any) => {
                    let result_id = 0;
                    if (Save_status && Save_status[0] && Save_status[0][0]) {
                        result_id = Number(Save_status[0][0].Account_Group_Id_);
                    }
                    ...
                    if (result_id > 0) {
                        ...
                        const dialogRef = this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Group Already Exists', Type: "2" } });
                    } else {
                        const dialogRef = this.dialogBox.open(DialogBox_Component, { panelClass: 'Dialogbox-Class', data: { Message: 'Error Occured', Type: "2" } });
                    }
                    this.issLoading = false;
    ...
    Edit_Account_Group(Account_Group_e: Account_Group, index) {
        ...
        this.Account_Group_Temp = new Account_Group();

...
@Component({
        selector: 'app-Vat_Report',
        templateUrl: './Vat_Report.component.html',
        styleUrls: ['./Vat_Report.component.css'],
        providers: [
                {provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE]},
                {provide: MAT_DATE_FORMATS, useValue: MY_FORMATS},
              ],
})
export class Vat_ReportComponent implements OnInit {
...
Search_VatReport()
        ...
        this.issLoading=true;
         debugger
        this.Receipt_Voucher_Service_.Search_VatReport(moment(this.Search_FromDate).format('YYYY-MM-DD'),
        moment(this.Search_ToDate).format('YYYY-MM-DD'),this.User_Type_Id, this.Login_User ).subscribe(Rows => {
                
    debugger
        this.VatReport_Data=Rows[0];
        this.Total_Entries=this.VatReport_Data.length;
        
        if (Rows != null) {
                this.Client_Accounts_Data_ = Rows[0];
                this.Print_Client1_ = this.Client_Accounts_Data_[0];           
            }
        if(this.VatReport_Data.length==0)
        {

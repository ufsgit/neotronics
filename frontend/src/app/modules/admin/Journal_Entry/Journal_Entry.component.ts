...
constructor(public Receipt_Voucher_Service_:Receipt_Voucher_Service,public Payment_Voucher_Service_:Payment_Voucher_Service,public Journal_Entry_Service_:Journal_Entry_Service, private route: ActivatedRoute, private router: Router,public dialogBox: MatDialog) { }
ngOnInit() 
{
...
{ var look_In_Date_Value=0,ClientAccount=0,Voucher_No_search_=0,ClientAccounts_Id=0;
    ...
    this.issLoading=true;
this.Journal_Entry_Service_.Search_Journal_Entry(
    moment(this.Search_FromDate).format('YYYY-MM-DD'), 
    moment(this.Search_ToDate).format('YYYY-MM-DD'),
    ClientAccount,ClientAccounts_Id,Voucher_No_search_,look_In_Date_Value,1,1).subscribe(Rows => {
...
Save_Journal_Entry()
{
    ...
    console.log("Before Journal Entry Save API call");
    this.issLoading = true;

    this.Journal_Entry_Service_.Save_Journal_Entry(this.Journal_Entry_)
    .pipe(
      finalize(() => {
        console.log("Journal Entry Save Finalize executed");
        this.issLoading = false;
    ...
    .subscribe({
      next: (Save_status) => {
        console.log("Journal Entry Save API Response:", Save_status);
      ...
      error: (error) => {
        console.error("Journal Entry Save API ERROR:", error);
        this.dialogBox.open(DialogBox_Component, {
          panelClass: 'Dialogbox-Class',

...
get Paginated_Price_Response_Master_Data(): Price_Response_Master[] {
    const source = this.Price_Response_Master_Data || [];
    const start = this.Page_Index * this.Page_Size;
    return source.slice(start, start + this.Page_Size);
}
get Price_Response_Total_Pages(): number {
    const total = this.Total_Entries || (this.Price_Response_Master_Data || []).length;
    return Math.max(1, Math.ceil(total / this.Page_Size));
}
get Price_Response_Page_Start(): number {
    if (!this.Total_Entries) return 0;
    return this.Page_Index * this.Page_Size + 1;
}
get Price_Response_Page_End(): number {
    return Math.min((this.Page_Index + 1) * this.Page_Size, this.Total_Entries || 0);
}
Previous_Price_Response_Page() {
    if (this.Page_Index > 0) this.Page_Index--;
}
Next_Price_Response_Page() {
    if (this.Page_Index < this.Price_Response_Total_Pages - 1) this.Page_Index++;
}
Add_Sales_Details()
{ 
